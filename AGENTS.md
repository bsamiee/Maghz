...
MCP servers extend reach without owning truth. Forge's `mcp-fleet.nix` is the sole registration and projection owner; Maghz carries no client configuration, MCP generator, or MCP CLI. The Forge fleet reaches Maghz through the read-only `doppler-remote` row, while `admin/infra.py` owns the remote `maghz-mcp` container. `heptabase-mcp` is the interactive Heptabase surface — whiteboard search, card reads, journal appends mid-conversation — while the `heptabase` CLI owns batch content custody and every deterministic write. Resolve external APIs through their fleet tools before findings bind, then promote results into schema, routines, or CLI behavior: deterministic work — schema apply, ledger mutations, synchronization, and stack lifecycle — always routes through `maghz`, never an MCP.
...
Gemini second-perspective and visual-judgment legs route through the `agy` skill (`.claude/skills/agy`), strongest reasoning tier pinned; its review lanes are read-only. Maghz remote may bootstrap the `agy` binary for parity, but Antigravity auth remains local-first until an official portable/headless credential flow is verified. Do not copy opaque `~/.gemini` auth state to the VPS by default.
