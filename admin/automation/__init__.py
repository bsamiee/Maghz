...
`AutomationSpec` wire record, the `AutomationReceipt` typed receipt, and the closed `Gate`/`GateReason`
admission vocabulary; faults ride the one `BoundaryFault` family. `engine.py` owns the single
polymorphic `drive(spec, cfg) -> Envelope` entrypoint, the
`decode_spec` admission converter, and the anyio lane coordination. This package adds no re-export
surface: `admin.rails` is the one CLI mount barrel and carries the public `drive` re-export alongside
the other rail callables, while `admin.__main__` binds the `AutomationSpec` decoder and `decode_spec`
